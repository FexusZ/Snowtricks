...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager, $upload_directory)
    {
        $client = new Client();
        $client->setFirstName('Admin')
        	->setLastName('Admin')
        	->setEmail('admin@admin.fr')
        	->setPassword($this->encode->encodePassword($client, 'admin1234'))
       	;
       	$manager->persit($client);
        $manager->flush();
        ...
        foreach ($data as $value) {
    		...
    		foreach ($value[4] as $image) {
    			$images = new Image();
    			file_put_contents($upload_directory . '/' . $image[1], file_get_contents($image[0]));
    			$images->setImage($image[1]);
    			$figure->addImages($images);
    		}


    		foreach ($value[5] as $video) {
    			$videos = new Video();
